...
This repo ships Codex marketplace metadata at `.agents/plugins/marketplace.json`. Codex installs the plugin package from `plugins/hivemind/`, whose `.codex-plugin/plugin.json` discovers skills from `plugins/hivemind/skills/`.
...
The Codex skill contents mirror the ones Claude Code uses, but without relying on Claude-only packaging. If you prefer a skill-only setup, install the root `skills/hivemind` from this repo as a Codex skill.
...
Once installed in Codex, the skill can invoke its packaged scripts from `plugins/hivemind/skills/hivemind/scripts/`.
